...
class Db:
    ...
    async def in_channel(self, user_id: int, chat_id: int, thread_id=None) -> bool:
       query = {"user_id": int(user_id), "chat_id": int(chat_id), "thread_id": thread_id}
       channel = await self.chl.find_one(query)
       return bool(channel)

    async def add_channel(self, user_id: int, chat_id: int, title, username, type="channel", thread_id=None):
       channel = await self.in_channel(user_id, chat_id, thread_id)
       if channel:
         return False
       return await self.chl.insert_one({"user_id": user_id, "chat_id": chat_id, "title": title, "username": username, "type": type, "thread_id": thread_id})

    async def remove_channel(self, user_id: int, chat_id: int, thread_id=None):
       channel = await self.in_channel(user_id, chat_id, thread_id)
       if not channel:
         return False
       return await self.chl.delete_many({"user_id": int(user_id), "chat_id": int(chat_id), "thread_id": thread_id})

    async def get_channel_details(self, user_id: int, chat_id: int, thread_id=None):
       channel = await self.chl.find_one({"user_id": int(user_id), "chat_id": int(chat_id), "thread_id": thread_id})
       if channel:
          channel.setdefault("type", "channel")
          channel.setdefault("thread_id", None)
       return channel
