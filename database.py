...
class Db:
    ...
    async def set_capture_mode(self, user_id, enabled, mode="destination", expiry_seconds=600):
       import time as _time
       key = {"user_id": int(user_id), "mode": mode}
       if enabled:
    ...
    async def is_capture_active(self, user_id, mode="destination") -> bool:
       import time as _time
       doc = await self.capture.find_one({"user_id": int(user_id), "mode": mode})
    ...
    async def set_source(self, user_id, chat_id, title, username, type="channel", thread_id=None):
    ...
    async def get_source(self, user_id):
        source = await self.src.find_one({'user_id': int(user_id)})
        if source:
            source.setdefault("type", "channel")
            source.setdefault("thread_id", None)
        return source
