...
- 📌 **Auto-pin first message** — the first message successfully forwarded/copied in a range is automatically pinned in the destination (channel/group/topic). Retries briefly on transient failures; if pinning ultimately fails (e.g. missing "Pin Messages" permission), the bot sends a one-time warning but forwarding always continues regardless.
- ❤️ **Completion notice** — once a full range finishes forwarding, the bot sends a short "That's it ❤️" message to the destination chat/topic.
...
├── Procfile                 # Process command for Heroku/Render
...
| `PING_URL` | ❌ No | *(auto)* | Keep-alive self-ping target URL. Render auto-injects `RENDER_EXTERNAL_URL` for this — no manual setup needed there. Leave empty on platforms without a public URL (e.g. Google Colab); the keep-alive task then logs a warning and exits harmlessly instead of pinging. |
| `PING_INTERVAL` | ❌ No | `540` | Seconds between keep-alive self-pings (only relevant if `PING_URL` is set/auto-detected). |
...
This bot is **polling-based** (uses Pyrogram's `idle()`, not a Telegram webhook), so it doesn't strictly need a public URL to function. The included aiohttp server only exists as a health-check endpoint for platforms (like Render's free tier) that require the process to bind a port. It supports **Render, Heroku, Koyeb, Railway, Google Cloud Run, Google Colab, VPS, and Termux**.

### One-Click Deploy

| Platform | Deploy |
|---|---|
| Render | [![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/cosmos475/aesthetic-project) |
| Heroku | [![Deploy to Heroku](https://www.herokucdn.com/deploy/button.svg)](https://heroku.com/deploy?template=https://github.com/cosmos475/aesthetic-project) |
| Koyeb | [![Deploy to Koyeb](https://www.koyeb.com/static/images/deploy/button.svg)](https://app.koyeb.com/deploy?type=git&repository=github.com/cosmos475/aesthetic-project&branch=main&name=vj-forward-bot) |
| Google Colab | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/cosmos475/aesthetic-project/blob/main/colab_deploy.ipynb) |
| Google Cloud | [![Open in Cloud Shell](https://gstatic.com/cloudssh/images/open-btn.svg)](https://ssh.cloud.google.com/cloudshell/editor?cloudshell_git_repo=https://github.com/cosmos475/aesthetic-project&cloudshell_tutorial=.cloudshell/GCLOUD.md) |
...
- The bot **does** include a built-in keep-alive self-pinger (`keep_alive()` in `app.py`) that pings `PING_URL` (auto-detected from Render's `RENDER_EXTERNAL_URL`, or set manually) roughly every `PING_INTERVAL` seconds — but only while a forward job is actively running (`temp.forwardings > 0`). It's designed to reduce idle sleep during long forward jobs, not to keep the service alive indefinitely with nothing happening.
...
Uses the included `app.json` and `Procfile`. After clicking the badge above, fill in the prompted fields. `PING_URL` can be left empty — Render-style auto-detection doesn't apply here, so keep-alive pinging is simply skipped unless you set it manually.
...
Click the Colab badge above to open `colab_deploy.ipynb`. Fill in the mandatory fields (`API_ID`, `API_HASH`, `BOT_TOKEN`, `BOT_OWNER`, `DATABASE_URI`) — optional fields (`DATABASE_NAME`, `BOT_SESSION`) come pre-filled — and run the single cell. It clones the repo, installs dependencies, and runs `python3 app.py`. `PING_URL` is intentionally left unset since Colab has no stable public URL; the keep-alive task logs a warning and exits harmlessly instead of pinging. The cell blocks and streams logs live; press ■ to stop.
...
```bash
git clone https://github.com/cosmos475/aesthetic-project.git
cd aesthetic-project
python3 -m venv venv
...
sudo docker build -t vj-forward-bot .
sudo docker run -it --rm --env-file .env vj-forward-bot
```
...
> If any MongoDB-related package fails to build on Termux, run `pkg install libffi openssl` first, then retry `pip install -r requirements.txt`. A remote MongoDB instance (e.g. [MongoDB Atlas](https://www.mongodb.com/atlas)'s free tier) is recommended over trying to run MongoDB on-device.
