...
async def settings_query(bot, query):
  ...
  elif type.startswith("branding") and int(user_id) != Config.BOT_OWNER:
     return await query.answer("owner only", show_alert=True)

  elif type=="branding":
     branding = await db.get_branding()
     status = "✅ ON" if branding['enabled'] and branding['url'] else "❌ OFF"
     buttons = [[
          InlineKeyboardButton(f'Status: {status}',
                       callback_data="settings#branding_toggle")
          ],[
          InlineKeyboardButton('🔗 Set Link',
                       callback_data="settings#branding_seturl")
          ]]
     if branding['url']:
        buttons.append([InlineKeyboardButton('❌ Remove Link',
                       callback_data="settings#branding_removeurl")])
     buttons.append([InlineKeyboardButton('back',
                      callback_data="settings#main")])
     await query.message.edit_text(
        f"<b><u>BRANDING</b></u>\n\n<b>Controls the optional Developer button shown on the main menu.</b>\n\n<b>Current link:</b> <code>{branding['url'] or 'not set'}</code>",
        reply_markup=InlineKeyboardMarkup(buttons))

  elif type=="branding_toggle":
     branding = await db.get_branding()
     if not branding['url']:
        return await query.answer("set a link first", show_alert=True)
     await db.set_branding(enabled=not branding['enabled'])
     await _refresh_branding(bot, query)

  elif type=="branding_seturl":
     await query.message.delete()
     ask = await bot.ask(user_id, text="<b>Send the Telegram link for the Developer button (channel/group/user).\nExample: https://t.me/yourusername</b>\n/cancel - cancel this process")
     if ask.text == '/cancel':
        return await ask.reply_text("<b>process canceled</b>", reply_markup=InlineKeyboardMarkup(buttons))
     link = ask.text.strip()
     if not (link.startswith("https://t.me/") or link.startswith("http://t.me/") or link.startswith("t.me/")):
        return await ask.reply_text("<b>Invalid link. Must be a t.me link</b>", reply_markup=InlineKeyboardMarkup(buttons))
     await db.set_branding(url=link, enabled=True)
     back_btn = [[InlineKeyboardButton('back', callback_data="settings#branding")]]
     await ask.reply_text("<b>successfully updated and enabled</b>", reply_markup=InlineKeyboardMarkup(back_btn))

  elif type=="branding_removeurl":
     await db.set_branding(url=None, enabled=False)
     await _refresh_branding(bot, query)
...
async def _refresh_branding(bot, query):
   branding = await db.get_branding()
   status = "✅ ON" if branding['enabled'] and branding['url'] else "❌ OFF"
   buttons = [[
        InlineKeyboardButton(f'Status: {status}',
...
def main_buttons(user_id=None):
  ...
  if user_id is not None and int(user_id) == Config.BOT_OWNER:
     buttons.append([InlineKeyboardButton('🏢 Bʀᴀɴᴅɪɴɢ',
                    callback_data=f'settings#branding')])
