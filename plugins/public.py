...
async def run(bot, message):
    ...
    temp.FWD_SETUP[user_id] = {"chat_id": chat_id, "title": title, "src_username": src_username, "src_type": src_type, "src_thread_id": src_thread_id, "toid": toid, "to_title": to_title, "thread_id": thread_id, "_bot": _bot}
...
# The old regex only matched the private "c/<internal_id>/..." form, so:
#   1) any topic-group link (which always has the extra thread_id segment) silently failed to
#      match and returned None -- topic-source Range Forward showed "Invalid message reference".
#   2) any public group/channel link (no "c/", uses the username instead) also failed to match --
#      public-source Range Forward showed "Invalid message reference" even though the source
#      itself was set up correctly and the bot could read it fine.
# Kept as two separate patterns (rather than one merged regex) so the private c/-link behaviour,
# which is already relied upon for private groups/topics, is not touched at all.
...
def _parse_msg_link(text):
    ...
    Unchanged from before -- still only handles the private c/ form.
    """
    if not text:
        return None, None
...
def _parse_public_msg_link(text):
    ...
    3-segment link (username/thread_id/message_id) -> topic message, thread_id set.
    2-segment link (username/message_id) -> normal message, thread_id is None.
    """
    if not text:
...
async def _extract_msg_id(msg, expected_chat_id, src_type="channel", expected_thread_id=None, expected_username=None):
    ...
    this is what allows Range Forward to work for public groups where the bot isn't an
    admin (so the source's chat_id was never resolvable from a c/-style link to begin with).
    """
