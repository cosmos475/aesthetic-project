...
async def run(bot, message):
    ...
    temp.FWD_SETUP[user_id] = {"chat_id": chat_id, "title": title, "src_type": src_type, "toid": toid, "to_title": to_title, "thread_id": thread_id, "_bot": _bot}
...
async def forward_mode(bot, query):
    ...
    if mode == "latest":
        ...
        skip = await _extract_msg_id(first_msg, chat_id, src_type)
        if skip is None:
        ...
        last_msg_id = await _extract_msg_id(last_msg, chat_id, src_type)
...
async def _extract_msg_id(msg, expected_chat_id, src_type="channel"):
