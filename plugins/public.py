...
async def wait_reply(bot, chat_id, text, timeout=120, **kwargs):
    ...
    Registers a one-off, dedicated handler at group=-999 for the duration of the wait
    instead of relying on pyropatch's bot.ask() (whose global checker=True resolver at
    group=-100 unconditionally stop_propagation()s and can silently swallow updates in
    the gap between one .ask() resolving and the next one registering -- causing the
    "no response" hang seen with back-to-back forwards). A dedicated handler per call
    avoids all shared-state timing races between concurrent/sequential waits.
    """
    await bot.send_message(chat_id, text, **kwargs)
    loop = asyncio.get_event_loop()
    fut = loop.create_future()

    async def _collector(client, message):
        if not fut.done():
            fut.set_result(message)

    handler = bot.add_handler(
        MessageHandler(_collector, filters.chat(chat_id) & filters.private),
        group=-999
    )
    await asyncio.sleep(0.3)  # let Pyrogram's async add_handler task actually finish registering
    try:
