...
async def run(bot, message):
    ...
    if not destinations:
       return await message.reply_text("please add a destination in Destination Setup before forwarding")
    if len(destinations) > 1:
       for d in destinations:
          buttons.append([KeyboardButton(f"{d['title']}")])
          btn_data[d['title']] = d['chat_id']
       ...
       toid = destinations[0]['chat_id']
       to_title = destinations[0]['title']
    ...
    temp.FWD_SETUP[user_id] = {"chat_id": chat_id, "title": title, "toid": toid, "to_title": to_title, "_bot": _bot}
...
async def forward_mode(bot, query):
    ...
    STS(forward_id).store(chat_id, toid, int(skip), int(last_msg_id))
