...
async def source_type_query(bot, query):
    ...
    if action == "group":
        ...
        await query.message.edit_text(
            ...
            "──────────────\n\n"
            "🌐 <b>Group is PUBLIC?</b>\n"
            "Paste any message link from that group below instead of "
            "adding this bot as admin (e.g. <code>https://t.me/username/67</code>).\n\n"
            "If you can't make this bot or your own bot admin here, add a Userbot "
            "first from Settings, then come back and set up the source with the link.\n\n"
    ...
    if action == "topic":
        ...
        await query.message.edit_text(
            ...
            "──────────────\n\n"
            "🌐 <b>Group is PUBLIC?</b>\n"
            "Paste that topic's message link below instead of adding this bot as admin "
...
async def _await_public_group_link(bot, user_id, src_type):
    ...
    Resolves the group via the user's own added Bot or Userbot (not this bot's own
    client) -- Telegram does not let a plain bot account resolve a Normal Group's public
    username unless it's already a member, but it does allow this for a Userbot session,
    and for the user's own bot once it's a member/admin. Falls back to this bot's own
    client only if the user hasn't added a Bot/Userbot yet, so the error message is
    still meaningful.
    """
    try:
    ...
    # Resolve using the user's own added Bot/Userbot -- see docstring for why.
    resolver, resolver_is_own_client = await _get_resolver_client(user_id, bot)
    try:
        chat = await resolver.get_chat(chat_id)
        title = chat.title
        ...
        hint = (
            "\n\n<i>Tip: a plain Bot account often can't resolve a public Normal Group/Topic "
            "unless it's already a member. Adding a Userbot in Settings usually fixes this.</i>"
            if resolver_is_own_client else ""
        )
        return await msg.reply_text(f"<b>Error:</b> {e}{hint}")
    finally:
        if resolver_is_own_client and resolver is not bot:
            try:
                await resolver.stop()
            except Exception:
                pass
...
async def _get_resolver_client(user_id, bot):
    """Returns (client, is_own_client_flag) to use for resolving a public group/topic
    username during source setup. Prefers the user's own added Userbot (works for public
    Normal Groups without membership), then their own added Bot (works once it's a
    member/admin), then falls back to this bot's own client (rarely works for Normal
    Groups, but kept as a last resort so the error message is still useful).
    """
    userbot_data = await db.get_userbot(user_id)
    if userbot_data:
        try:
            client = await get_client(userbot_data['session'], is_bot=False)
            await client.start()
            return client, True
        except Exception:
            pass
    ...
    if bot_data:
        try:
            client = await get_client(bot_data['token'], is_bot=True)
            await client.start()
            return client, True
        except Exception:
            pass
