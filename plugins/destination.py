...
# pyrofork assigns ChatType.FORUM (not GROUP/SUPERGROUP) to supergroups that have
# Topics enabled. filters.group only matches GROUP/SUPERGROUP, so any update from a
# forum-enabled supergroup was being silently dropped before reaching any handler
# below. any_group_or_forum covers all three so /setdestination works in topic groups too.
...
@Client.on_message(any_group_or_forum | filters.channel, group=-1)
...
async def destination_query(bot, query):
    ...
    if action == "topic":
        await db.set_capture_mode(user_id, True, mode="destination")
        return await query.message.edit_text(
...
@Client.on_message(filters.command("setdestination") & (any_group_or_forum | filters.channel))
async def set_destination_topic(client, message):
    ...
    try:
        if thread_id:
            if not await db.is_capture_active(user_id, mode="destination"):
            ...
            await db.set_capture_mode(user_id, False, mode="destination")
            return await client.send_message(
                chat_id,
        ...
        print(f"[SETDEST][ERROR] {type(e).__name__}: {e}")
        try:
            await client.send_message(chat_id, f"⚠️ Error saving destination: <code>{e}</code>", message_thread_id=thread_id)
        except Exception as e2:
            print(f"[SETDEST][ERROR-FALLBACK-FAILED] {type(e2).__name__}: {e2}")
