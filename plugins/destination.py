...
async def destination_query(bot, query):
    ...
    if action == "list":
        ...
        for d in destinations:
            ...
            tid = d.get("thread_id") if d.get("thread_id") is not None else "none"
            buttons.append([InlineKeyboardButton(f"{label} {d['title']}",
                            callback_data=f"dest#view_{d['chat_id']}_{tid}")])
    ...
    if action.startswith("view_"):
        ...
        d = await db.get_channel_details(user_id, chat_id, thread_id)
        label = TYPE_LABEL.get(d.get("type", "channel"), "📢 Channel")
        text = (
        ...
        tid = thread_id if thread_id is not None else "none"
        buttons = [[
            InlineKeyboardButton('❌ Remove', callback_data=f"dest#remove_{chat_id}_{tid}")
        ],[
    ...
    if action.startswith("remove_"):
        ...
        chat_id = parts[1]
        thread_id = None if parts[2] == "none" else int(parts[2])
        await db.remove_channel(user_id, chat_id, thread_id)
