...
async def set_destination_topic(client, message):
    ...
    if not message.chat.is_forum or not message.message_thread_id:
        return await message.reply("<b>This command only works inside a forum topic.</b>")
    ...
    await message.reply_text(
        "<b>Successfully added this topic as a destination</b>" if added else "<b>This topic is already added</b>"
    )
