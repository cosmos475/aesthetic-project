...
async def pub_(bot, message):
    ...
    try: 
       # Warm up peer resolution first (helps username-based public sources resolve
       # properly before get_messages() is called on a fresh client).
    ...
    if locked:
        try:
          ...
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=sts.get("skip"), filters=filter, max_size=max_size):
                ...
                if forward_tag:
                   ...
                   new_caption, new_entities, is_html = custom_caption(message, caption)
                   details = {"msg_id": message.id, "media": media(message), "caption": new_caption, "caption_entities": new_entities, "caption_is_html": is_html, 'button': button, "protect": protect}
                   ok = await copy(user, client, details, m, sts)
                   if ok:
                      sts.add('total_files')
        ...
        try:
            await client.send_message(
                chat_id=sts.get('TO'),
                text="That's it ❤️",
                message_thread_id=sts.get('thread_id'))
        except Exception as e:
            print(f"Could not send completion message to destination: {e}")
...
async def pin_first_message(bot, user, m, sts, sent_message):
    """Pins the first successfully forwarded/copied message in the
    destination chat (topic-aware via message_thread_id). Retries a
    couple of times on transient errors, but never blocks or aborts
    the forwarding process — pinning is best-effort, forwarding always
    takes priority. Notifies the user once if it ultimately fails
    (e.g. missing Pin Messages permission)."""
    if sent_message is None:
        return
    if sts.data[sts.id].get('pinned_done'):
        return
    # Mark done immediately so a retry storm or a race between concurrent
    # sends can't trigger multiple pin attempts for the "first" message.
    sts.data[sts.id]['pinned_done'] = True

    thread_id = sts.get('thread_id')
    chat_id = sts.get('TO')
    last_error = None
    ...
    print(f"Auto-pin failed for chat {chat_id}: {last_error}")
    try:
        await send(bot, user, "⚠️ Auto pin failed because bot has not pin rights or permission.")
    except Exception:
        pass
...
async def copy(user, bot, msg, m, sts):
   try:
     ...
     is_template_merged = msg.get("caption_is_html")
     # Two distinct paths:
     # 1) No custom template: caption is the ORIGINAL plain text + its
     #    original entities (or no entities). We never set parse_mode here,
     #    so literal <, >, & in the text are always safe as plain text and
     #    entities (if any) render exactly as in the source.
     # 2) Custom template merged: caption is an HTML string (built from
     #    html.escape()'d parts), so parse_mode=HTML is required to render
     #    the formatting/links correctly.
     parse_mode = ParseMode.HTML if is_template_merged else None
     if msg.get("media") and msg.get("caption"):
        ...
        sent_message = await bot.copy_message(
              ...
              protect_content=msg.get("protect"),
              message_thread_id=thread_id)
     await pin_first_message(bot, user, m, sts, sent_message)
     return True
   except FloodWait as e:
...
async def forward(user, bot, msg, m, sts, protect):
   try:                             
     thread_id = sts.get('thread_id')
     sent_messages = await bot.forward_messages(
           chat_id=sts.get('TO'),
           from_chat_id=sts.get('FROM'), 
           protect_content=protect,
           message_ids=msg,
           message_thread_id=thread_id)
     first_sent = sent_messages[0] if isinstance(sent_messages, list) else sent_messages
     await pin_first_message(bot, user, m, sts, first_sent)
...
async def edit(user, msg, title, status, sts):
   ...
   expected_total = getattr(i, 'expected_total', None) or i.total or 1
   current = max(0, i.fetched - i.skip)  # fetched starts at skip (offset-based), normalize to 0-based count
   percentage = min(100, round(float(current) * 100 / float(expected_total)))
   now = time.time()
   diff = int(now - i.start)
   speed = sts.divide(current, diff)
   time_to_completion = round(sts.divide(expected_total - current, int(speed))) if status == 'Forwarding' else 0
   eta = _format_eta(time_to_completion) if status == 'Forwarding' else "-"
   text = TEXT.format(current, expected_total, i.total_files, i.duplicate, i.deleted, i.filtered, status, percentage, eta, title)
   await update_forward(user_id=user, last_id=None, start_time=i.start, limit=i.limit, chat_id=i.FROM, toid=i.TO, forward_id=None, msg_id=msg.id, fetched=i.fetched, deleted=i.deleted, total=i.total_files, duplicate=i.duplicate, skip=i.skip, filterd=i.filtered, thread_id=getattr(i, 'thread_id', None))
   if status in ["cancelled", "completed"]:
      button = None
   else:
      button = InlineKeyboardMarkup([[InlineKeyboardButton('• ᴄᴀɴᴄᴇʟ', 'terminate_frwd')]])
  ...
  if msg.media:
    if (msg.video or msg.document or msg.audio or msg.photo):
      ...
      if media:
        ...
        fentities = getattr(msg, 'caption_entities', None)
        if caption:
          # Custom template is being merged in as plain text, so we must use
          # the HTML-rendered version of the original caption here (entities
          # can't be spliced into a plain-text template). html.escape guards
          # against literal <, >, & in either the template or the caption
          # breaking HTML parsing / silently truncating the message.
          import html as _html
          safe_fcaption = fcaption.html if fcaption else ''
          try:
            merged = caption.format(filename=_html.escape(str(file_name)), size=_html.escape(get_size(file_size)), caption=safe_fcaption)
          except Exception:
            merged = safe_fcaption
          return merged, None, True
        # No custom template: return the ORIGINAL caption text + its
        # original entities untouched. This avoids the .html round-trip
        # entirely, so literal <, >, & in the caption can never be
        # misparsed as HTML tags and truncate/break formatting.
        return fcaption, fentities, False
...
async def restart_pending_forwads(bot, user):
    ...
    try:
       ...
       if fetch > 0:
           # Some messages were already forwarded before the restart, so
           # the first message of this range was almost certainly already
           # pinned. In-memory pin state doesn't survive a process
           # restart, so we assume-and-skip here rather than re-pinning
           # (which could otherwise override something the user pinned
       ...
       try:
          ...
          return await db.rmve_frwd(user)
       try: 
          # Same peer warm-up as the main pub_() handler -- see comment there.
          await client.get_chat(sts.get("FROM"))
          await client.get_messages(sts.get("FROM"), sts.get("limit"))
       except:
...
async def store_vars(user_id):
    ...
    print(fetch)
    STS(id=forward_id).store(settings['chat_id'], settings['toid'], settings['skip'], settings['limit'], settings.get('thread_id'))
    return forward_id
...
async def update_forward(user_id, chat_id, start_time, toid, last_id, limit, forward_id, msg_id, fetched, total, duplicate, deleted, skip, filterd, thread_id=None):
    details = {
        'chat_id': chat_id,
        'toid': toid,
