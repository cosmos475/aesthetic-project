...
async def pub_(bot, message):
    ...
    if locked:
        ...
        temp.IS_FRWD_CHAT.remove(sts.TO)
        i = sts.get(full=True)
        time_taken = _format_duration(int(time.time() - i.start))
        completion_text = Script.COMPLETION.format(
            forwarded=i.total_files, duplicate=i.duplicate, deleted=i.deleted,
            skipped=i.skip, filtered=i.filtered, first_id=i.skip, last_id=i.limit,
            time_taken=time_taken
        )
        await send(client, user, completion_text)
...
async def edit(user, msg, title, status, sts):
   ...
   raw_fetched = max(0, i.fetched - i.skip)  # fetched is offset-based (starts at skip), normalize to a 0-based count
   percentage = min(100, round(float(raw_fetched) * 100 / float(expected_total)))
   text_fetched_display = f"{raw_fetched}"
   ...
   speed = sts.divide(raw_fetched, diff)
   elapsed_time = round(diff) * 1000
   time_to_completion = round(sts.divide(expected_total - raw_fetched, int(speed))) * 1000
   estimated_total_time = elapsed_time + time_to_completion
   eta_display = _format_duration(time_to_completion / 1000) if status == 'Forwarding' else "-"
   text = TEXT.format(text_fetched_display, expected_total, i.total_files, i.duplicate, i.deleted, i.skip, i.filtered, status, percentage, eta_display, title)
