...
async def pub_(bot, message):
    ...
    try: 
       # Warm up peer resolution first. A freshly-created client (like this one) has an
       # empty local peer cache, so calling get_messages() directly on a username-based
       # chat_id can throw a local KeyError ("Username not found: ...") before Pyrogram
       # even gets to make the Telegram API call. get_chat() forces a proper resolve
       # (contacts.resolveUsername for public usernames, or a cache lookup for numeric
       # IDs the client already knows about from being added as admin) and populates the
       # cache, so the get_messages() call right after it can find the peer.
       await client.get_chat(sts.get("FROM"))
       await client.get_messages(sts.get("FROM"), sts.get("limit"))
    except Exception as e:
       print(f"[FWD-CHECK-ERROR] FROM={sts.get('FROM')} is_bot={_bot['is_bot']} {type(e).__name__}: {e}")
...
async def restart_pending_forwads(bot, user):
    ...
    try:
       ...
       try: 
          ...
          print(f"[FWD-CHECK-ERROR][restart] FROM={sts.get('FROM')} is_bot={_bot['is_bot']} {type(e).__name__}: {e}")
