...
╔════❰ ғᴏʀᴡᴀʀᴅ sᴛᴀᴛᴜs  ❱═❍⊱❁۪۪
║╭━━━━━━━━━━━━━━━➣
║┣⪼<b>🕵 ғᴇᴄʜᴇᴅ Msɢ :</b> <code>{}</code>/<code>{}</code>
║┃
...
<b>If the above is checked then the yes button can be clicked</b>"""
  ...
  RANGE_CONFIRM = """<b>📋 CONFIRM RANGE FORWARD</b>

<b>📢 Source:</b> `{from_chat}` ({src_label})
<b>📥 Destination:</b> `{to_chat}`

<b>🔢 Range:</b> <code>{first_id}</code> → <code>{last_id}</code>
<b>✉️ Messages to forward:</b> ~<code>{expected_total}</code>
<b>⏱ Delay:</b> <code>{delay}s</code> per message
<b>🕐 Estimated time:</b> ~<code>{eta}</code>

<i>° [{botname}](t.me/{botuname}) must be admin in the destination</i>
<i>° If the source is private, your userbot must be a member (or your bot must be admin) there too</i>

<b>Proceed?</b>"""

  COMPLETION = """<b>✅ Forwarding complete!</b>
